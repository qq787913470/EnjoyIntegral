				...
				<table class="b">
				    <tr align="center">
						...
						<input type="button" style="width: 80px" value="进入购物列表"
							 ...
							 购物车数量:<span style="color: red" id="count">${productCountInCart+1}</span>
						  </center>
						</td>
					</tr>
				</table>
						...
						<tbody>
							...
							<c:forEach items="${page.result}" var="item" varStatus="status">
								<tr class="${status.count % 2 == 0 ? 'odd' : 'even'}" >
									...
									<td><input type="button" value="加入购物车" onclick="putInCart(this,'${item.productId}','${item.productName}','${item.price}','${item.integralValue}','${item.giftCount}')"/>
									</td>
									<!--
...
<%-- 								<td >
									<c:choose>
									 <c:when test="${fn:contains(productIdsNomal,item.productId)}">
									    <span style="color: blue" ><c:out value='${item.merName}' />&nbsp;</span>
									 </c:when>
									 <c:otherwise>

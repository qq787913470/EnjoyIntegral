        ...
        <div class="biao">
            <table class="grid">
                ...
                <tr>
                    ...
                    <td class="lgridlist"><%=ProductEnjoy.ALIAS_PRODUCT_NAME%>
                    </td>
                    ...
                    <td colspan="4">
                        <center>
                            <input type="submit" style="width: 80px" value="查询"
                                   onclick="getReferenceForm(this).action='getActivityProduct.do'"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <div class="form">
            <div id="eXtremeTable" class="eXtremeTable">
                <table class="grid its b">
                    ...
                    <tr>
                        <th>购物车数量</th>
                        <th><%=ProductEnjoy.ALIAS_PIC%>
                        </th>
                        <th><%=ProductEnjoy.ALIAS_PRODUCT_NAME%>
                        </th>
                        <th><%=ProductEnjoy.ALIAS_INTEGRAL_VALUE%>
                        </th>
                        <th><%=ProductBase.ALIAS_ORG_NAME%>
                        </th>
                        <th><%=ActivityProductEnjoy.ALIAS_LIMIT_COUNT%>
                        </th>
                        <th><%=ActivityProductEnjoy.ALIAS_GIFT_COUNT%>
                        </th>
                        <th><%=ProductEnjoy.ALIAS_REMARK%>
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="${status.count % 2 == 0 ? 'odd' : 'even'}">
                            ...
                            <td>
                                <input type="button" value="+"
                                       onclick="putInCart(this,'${item.productId}','${item.productName}','${item.price}','${item.integralValue}','${item.limitCount}','${item.giftCount}')"/>
                                <!--预先设置的值-->
                                ...
                                <input type="button" value="-"
                                       onclick="outFromCart(this,'${item.productId}','${item.productName}','${item.price}','${item.integralValue}','${item.limitCount}','${item.giftCount}')"/>
                            </td><td>
...
<script type="text/javascript">
            ...
            if (restValue < costValue) {
                ...
                if (statArray.length > giftCount) {
                    ...
                    if (productIdStr.split(productId).length >= (parseInt(limitCount) + 1)) {
                        ...
                        item.parentNode.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.style.color = "blue";
                    }
                }
            }
        });
    ...
    function outFromCart(item, productId, productName, price, integralValue, limitCount, giftSum) {
        require(["dojo/_base/window"], function (win) {
            var count = dojo.byId("productCountInCart").value;
            var restIntegral = dojo.byId("restIntegral");
            var restIntegralShow = dojo.byId("restIntegralShow");
            ...
            if (count == 0) {
                ...
                var productCart = dojo.byId("productCart");
                var countSpan = dojo.byId("count");
                document.getElementById(productId).value = productIdStr.split(productId).length-2
                var tr = win.doc.createElement('tr');
                var inputId = win.doc.createElement('input');
                dojo.attr(inputId, "type", "hidden");
                dojo.attr(inputId, "name", "productIdInCart");
                ...
                productCart.appendChild(tr);
                var ids = productIdsActivity.value;
                productIdsActivity.value = ids.replace(productId + "@" + activityId + "-","");
                count = parseInt(count) - 1;
                countSpan.innerHTML = count;
                productCountInCart.value = count;
                restValue = restValue - costValue;

...
@Controller
public class OrgChangeSumReportController extends BaseController {

	@Autowired
	private OrgChangeSumReportManager orgChangeSumReportManager;
	@Autowired
	private OrgManager orgManager;

	@RequestMapping
	public String list(Model model, @ModelAttribute("query") OrgChangeSumReportQuery query) {
		...
		Page page = orgChangeSumReportManager.queryPage(query);
		//对每页的积分进行汇总
		List<OrgChangeSumReport> result = page.getResult();
		Double countIntegralValue=0.0 ;
		for(OrgChangeSumReport orgChangeSumReport:result){
			countIntegralValue = orgChangeSumReport.getAllAmt()+countIntegralValue;
		}
		...
		return "/enjoyIntegral_pages/OrgChangeSumReport/list";
	}
	@RequestMapping
	public ResponseEntity<byte[]> downloadErrorTranLsReport(Model model, @ModelAttribute("query") OrgChangeSumReportQuery query) throws IOException, org.apache.poi.openxml4j.exceptions.InvalidFormatException {
		String userInOrg = getCurUser().getUserExt().getOrgId();
		...
		List<OrgChangeSumReport> ModelList = orgChangeSumReportManager.fineAll(query);
		List<Map<String, String>> page = new ArrayList<>();
		Double integralCount = 0.0;
		for (OrgChangeSumReport m : ModelList) {
			integralCount = m.getAllAmt()+integralCount;
			page.add(BeanUtils.beanToMap(m));
		}
		StringUtils date = new StringUtils();
		String reportName = "机构兑换汇总报表_" + date.getStringToDate() + "_" + getCurUser().getLoginName();
		String[] reportLine = {
				"tranDate",
				"merNo",
				"merName",
		...
		return createExcel(integralCount,reportName, reportLine, ReportHeader.OrgChangeSumReportHeader, page);
	}
	/**
	 * Excel文件下载处理
	 *
	...
	private ResponseEntity<byte[]> createExcel(Double integralCount,String reportName,
		...
		InputStream in = OrgChangeSumReportController.class.getClassLoader().getResourceAsStream("reportSum.xls");

			...
			<div class="biao" id="mainContent">
				<div data-dojo-type="dijit/Dialog" data-dojo-id="myDialog"
					...
					<mvc:form id="queryForm" name="queryForm" action="saveProductStateReason.do"
						...
						<table class="grid">
							<input type="hidden" name="productId" value="${productBase.productId}"/>
							<input type="hidden" name="merNo" value="${productBase.merNo}"/>
							<input type="hidden" name="isGround"
								<c:if test="${productBase.isGround == '0'}">value="1"</c:if>
								<c:if test="${productBase.isGround == '1'}">value="0"</c:if>
							/>
							<tr>

...
<mvc:form id="queryForm" name="queryForm" action="getNomalProduct.do"
    ...
    <div id="content">
        ...
        <div class="biao">
            <table class="grid">
                <input type="hidden" id="certificateTypeIdHidden" name="certificateTypeIdHidden"
                       value="${certificateTypeIdHidden}"/>
                <input type="hidden" id="certificateNoHidden" name="certificateNoHidden"
                       value="${certificateNoHidden}"/>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" size="5" value="${query.integralValuestart}" id="integralValuestart"
                               name="integralValuestart" data-dojo-type="dijit.form.ValidationTextBox"
                               data-dojo-props="trim:true,required:false,regExp:'^([1-9]([0-9]+)?(\.[0-9]{1,2})?)|((0){1})|([0-9]\.[0-9]([0-9])?)$',invalidMessage:'请输入整数或小数(小数点后保留两位)'"/>
                        至
                        <input type="text" size="5" value="${query.integralValueend}" id="integralValueend"
                               name="integralValueend" data-dojo-type="dijit.form.ValidationTextBox"
                               data-dojo-props="trim:true,required:false,regExp:'^([1-9]([0-9]+)?(\.[0-9]{1,2})?)|((0){1})|([0-9]\.[0-9]([0-9])?)$',invalidMessage:'请输入整数或小数(小数点后保留两位)'"/>
                    </td>
                </tr>
                <tr>

                    <td class="lgridlist"><%=ProductBase.ALIAS_PRODUCT_NAME%>
                    </td>
                    <td><input type="text" value="${query.productName}" name="productName"
                    ...
                    <td colspan="4">
                        <center>
                            <input type="submit" style="width: 80px" value="查询"
                                   onclick="getReferenceForm(this).action='getNomalProduct.do'"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            ...
                            <input type="button" style="width: 80px" value="进入购物列表"
                                   onclick="putProductNomal('${ctx}/business/putProductNomal.do')"/>
        ...
        <div data-dojo-type="dijit/Dialog" data-dojo-id="myDialog" title="购物列表"
             style="font-size: 14pt; padding: 3px; color: #88863b !important; font-weight: bold;">
            <div id="eXtremeTable" class="eXtremeTable" name="eXtremeTable">
                <table class="grid its">
                    <thead>
                    <tr>
                        <th><%=ProductBase.ALIAS_PRODUCT_NAME%>
                        </th>
                        <th><%=ProductBase.ALIAS_PRICE%>
                        </th>
                        <th><%=ProductCart.ALIAS_COUNT%>
                        </th>
                        <th><%=ProductBase.ALIAS_INTEGRAL_VALUE%>
                        </th>
                    </tr>
                    </thead>
                    <tbody id="productCart" name="tbody">
            ...
            <table class="b">
                <tr align="center">
                    ...
                    <td><input type="button" style="width: 80px" value="进入购物列表"
                               onclick="putProductNomal('${ctx}/business/putProductNomal.do')"/></td>
                </tr>
            </table>
            ...
            <div id="eXtremeTable" class="eXtremeTable">
                <table class="grid its b">
                    ...
                    <tr>
                        ...
                        <th><%=ProductBase.ALIAS_PIC%>
                        </th>
                        <th><%=ProductBase.ALIAS_PRODUCT_NAME%>
                        </th>
                        <th><%=ProductBase.ALIAS_INTEGRAL_VALUE%>
                        </th>
                        <th><%=ProductBase.ALIAS_GIFT_COUNT%>
                        </th>
                        <th><%=ProductBase.ALIAS_ORG_NAME%>
                        </th>
                        <th><%=ProductBase.ALIAS_REMARK%>
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr class="${status.count % 2 == 0 ? 'odd' : 'even'}">
                            ...
                            <td>
                                <input type="button" value="+"
                                       onclick="putInCart(this,'${item.productId}','${item.productName}','${item.price}','${item.integralValue}','${item.giftCount}')"/>
                                <!--预先设置的值-->
                                ...
                                <c:forEach items="${products}" varStatus="status" var="pro">
                                    <c:if test="${pro.productId == item.productId}">
                                        ...
                                        <input id="${item.productId}" type="text" size="2" value="${pro.count}"
                                               readOnly="true"/>
                                    </c:if>
                                </c:forEach>
                                ...
                                <input type="button" value="-"
                                       onclick="outFromCart(this,'${item.productId}','${item.productName}','${item.price}','${item.integralValue}','${item.giftCount}')"/>
                            </td><td>
                                <c:if test="${item.picPath !=null}">
                                    <img id="imgPre" width="100" height="100" border=0
                                         src='../merchantbase/showImage.do?pic_addr=${item.picPath}'>
                                </c:if>
                                <c:if test="${item.picPath ==null}">
                                    <img id="imgPre" src="../images/noPic.jpg" style="display: block;" width="100"
                                         height="100"/>
                                </c:if>
                            </td><td>
                                <c:choose>
                                ...
                                <c:choose>
                                    <c:when test="${fn:contains(productIdsNomal,item.productId)}">
												<span style="color: blue"><c:out
                                                        value='${item.orgName}'/>&nbsp;</span>
                                    </c:when>
                                    <c:otherwise>
                                        <c:out value='${item.orgName}'/>&nbsp;
                                    </c:otherwise>
                                </c:choose>
                            </td><td title="${item.remark}" width="300px">
                                <c:choose>
                                    <c:when test="${fn:contains(productIdsNomal,item.productId)}">
...
<script type="text/javascript">
    ...
    function putInCart(item, productId, productName, price, integralValue, giftSum) {
        require(["dojo/_base/window"], function (win) {
            var count = dojo.byId("productCountInCart").value;
            var restIntegral = dojo.byId("restIntegral");//客户拥有的积分数
            var restIntegralShow = dojo.byId("restIntegralShow");
            var productCountInCart = dojo.byId("productCountInCart");
            var productIdsNomal = dojo.byId("productIdsNomal");//购物车中的商品，如何有多个商品就使用字符串拼接起来
            ...
            if (restValue < costValue) {
                ...
                if (productIdsNomal.value.split(productId).length >= (parseInt(giftSum) + 1)) {
                    alert("此商品库存已经没有了！！");
                } else {
                    var productCart = dojo.byId("productCart");
                    var countSpan = dojo.byId("count");
                    document.getElementById(productId).value = productIdsNomal.value.split(productId).length
                    ...
                    var ids = productIdsNomal.value;
                    productIdsNomal.value = ids + productId + "@nomal" + "-"
                    ...
                    item.parentNode.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.style.color = "blue";
                }
            }
        });
    }

    function outFromCart(item, productId, productName, price, integralValue, giftSum) {
        require(["dojo/_base/window"], function (win) {
            var count = dojo.byId("productCountInCart").value;
            var restIntegral = dojo.byId("restIntegral");//客户拥有的积分数
            var restIntegralShow = dojo.byId("restIntegralShow");
            var productCountInCart = dojo.byId("productCountInCart");
            var productIdsNomal = dojo.byId("productIdsNomal");//购物车中的商品，如何有多个商品就使用字符串拼接起来
            var curCount = document.getElementById(productId).value;
            var restValue = parseInt(restIntegral.value);
            var costValue = parseInt(integralValue);
            if (count == 0) {
                alert("购物车已经没有产品可以减少！！")
                ...
                var productCart = dojo.byId("productCart");
                var countSpan = dojo.byId("count");
                document.getElementById(productId).value = productIdsNomal.value.split(productId).length - 2
                var tr = win.doc.createElement('tr');
                var inputId = win.doc.createElement('input');
                dojo.attr(inputId, "type", "hidden");
                dojo.attr(inputId, "name", "productIdInCart");
                ...
                productCart.appendChild(tr);
                var ids = productIdsNomal.value;
                productIdsNomal.value = ids.replace(productId + "@nomal" + "-", "");
                count = parseInt(count) - 1;
                countSpan.innerHTML = count;
                productCountInCart.value = count;
                restValue = restValue + costValue;

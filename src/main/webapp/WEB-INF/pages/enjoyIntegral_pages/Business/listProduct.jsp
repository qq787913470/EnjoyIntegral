	...
	function printPage(data) {
	   /* var msse = data;
		...
		if (confirm('确定生成打印单吗？')) {
			...
			//var printContent = dojo.byId("printContent").innerHTML;
			...
			pageCode += data;
			//pageCode +='<input type="button" style="width: 80px" value="确定" onclick="javascript:window.print()"/>';  
			pageCode += '</div>';
			pageCode += '</body>';
			pageCode += '</html>';
		...
		dojo.xhrGet({
					...
					load : function(data) {
						if ("success" == data) {
							...
							/* dojo.attr("pushButton", "disabled", "disabled");
							dojo.style("pushButton",{"background": "url(../images/btn_bg2.jpg) repeat-x center"}); */
							...
							if(integral == "99" || integral=="" || integral == null){
                                ...
                                var phone = inputPhone.innerHTML;
                                //TODO 需要恢复的注释
		...
		<div id="content">
					...
					<table class="grid its">
						<thead>
							<tr>
							    <th>操作</th>
								<th>活动名称</th>
								<th><%=ProductCart.ALIAS_PRODUCT_NAME%></th>
								<th>购物车数量</th>
								<th><%=ProductCart.ALIAS_INTEGRAL_VALUE%></th>
							</tr>
						</thead>
						<tbody>
							 <c:forEach items="${productCarts}" var="item" varStatus="status">
								<tr>
								   <th><a href="${ctx}/business/removeProductInCart.do?productId=${item.productId}&activityId=${item.activityId}&certificateTypeIdHidden=${certificateTypeIdHidden}&certificateNoHidden=${certificateNoHidden}&restIntegral=${restIntegral}&productIdsNomal=${productIdsNomal}&productCountInCart=${productCountInCart}&productIdsActivity=${productIdsActivity}">移除购物车</a></th>
			...
			<table class="b">
				<tr align="center">
					...
					<td><input type="button" id="pushButton" style="width: 80px;background: url(../images/btn_bg2.jpg) repeat-x center;" value="刷卡"
			...
			<div data-dojo-type="dijit/Dialog" data-dojo-id="exitDialog"
				...
				<table class="b">
					<tr align="center">
						<td><center><input type="button" style="width: 80px" value="确定" onclick="exitPage()"/></center></td>
					</tr>
				</table>
			</div>
		</div>
		</center>
		<div id="maskpage" style="display:none; absolute; left:0; top:0; width: 100%; height:100%; overflow-y:scroll; background-color: black; z-index:9; -moz-opacity: 0.7; opacity:.70; filter: alpha(opacity=70);" ></div>
